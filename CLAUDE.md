...
- `login()` self-throttles via `SUNSYNK_LOGIN_RATE_LIMIT_SECONDS` (60s) and raises `LoginRateLimitedException`. The internal 401 refresh calls `login(force=True)` to bypass the throttle — a token refresh is not a retry of a failed login.
